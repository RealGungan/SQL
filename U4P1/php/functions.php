...
function login($conn, $password, $usrname)
{
    try {
        ...
        setcookie('generic', $dni[0][0], time() + (86400 * 30), "/");

        $link = "<script>window.open('../index_cliente.html')</script>";
        echo $link;
    } catch (PDOException $e) {
        echo "<br>Error: " . $e->getMessage();
    }
...
//         if ($key != 'SLG_G_WPT_TO' && $key != 'SLG_GWPT_Show_Hide_tmp' && $key != 'SLG_wptGlobTipTmp') {
